...
@NamedQueries({
    ...
    , @NamedQuery(name = "CtactemovimientodetalleView.findByRefDocumento", query = "SELECT d,c.fecha FROM CtactemovimientodetalleView d INNER JOIN CtactemovimientoView c ON d.idctactemovimiento = c.idctactemovimiento WHERE (c.ctacte like :search or LOWER(c.ctactenombre) like LOWER(:search) or TRIM(c.nro)= :searchExact)  and c.iddocumento = :iddocumento order by c.fecha desc")
