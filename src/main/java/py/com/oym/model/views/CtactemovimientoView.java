...
@NamedQueries({
    ...
    @NamedQuery(name = "CtactemovimientoView.findByRefDocumento", query = "SELECT c FROM CtactemovimientoView c WHERE (c.ctacte = :search or LOWER(c.ctactenombre) like LOWER(:search) or c.nro= :search)  and c.iddocumento = :iddocumento order by c.fecha desc") ,   
    @NamedQuery(name = "CtactemovimientoView.findByRefDocumentoFecha", query = "SELECT c FROM CtactemovimientoView c WHERE (c.ctacte = :search or LOWER(c.ctactenombre) like LOWER(:search) or c.nro= :search)  and c.iddocumento = :iddocumento and c.fecha between :fechaini and :fechafin order by c.fecha desc") ,   
